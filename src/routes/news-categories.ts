...
app.delete("/:id", authMiddleware, async (c) => {
    try {
        ...
        // Check if category is used by any news articles (via FK or legacy text field)
        const [usageResult] = await db
            .select({ count: count() })
            .from(news)
            .where(or(
                eq(news.categoryId, id),
                eq(news.category, existing.name)
            ));
        if (usageResult.count > 0) {
            return c.json({
                success: false,
                error: `Kategori "${existing.name}" tidak dapat dihapus karena masih digunakan oleh ${usageResult.count} berita.`,
                inUse: true,
            }, 409);
        }
